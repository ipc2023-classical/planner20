...
std::vector<std::shared_ptr<PartialAssignment>> random_modified_tasks = std::vector<std::shared_ptr<PartialAssignment>>();

class SuiNode;
class SuiNodePtrCompare;

int random_samples = 0;
float random_samples_percentage = 0.0;
string random_estimates = "default";
trie::trie<std::shared_ptr<PartialAssignment>> trie;
std::unordered_map<size_t,SuiNode> sui_mapping;
double max_time = -1;
...
SamplingTechnique::SamplingTechnique(const options::Options &opts)
    ...
    sampling_technique::random_samples_percentage = random_percentage;

    assert(random_percentage >= 0.0 && random_percentage < 1.0);
    if (random_percentage > 0.0) {
    ...
    sampling_technique::max_time = max_time;
}
...
vector<shared_ptr<PartialAssignment>> SamplingTechnique::next_all(
    ...
    if (sampling_technique::random_samples != 0 && samples.size() != (unsigned)max_samples) {
        float pct_sampled = (float)samples.size() / max_samples;
        utils::g_log << (100 * pct_sampled) << "% of budget states were sampled. "
            << "Updating number of random samples for the same ratio." << endl;
        sampling_technique::random_samples = round(sampling_technique::random_samples * pct_sampled);
    }
    return samples;
}
