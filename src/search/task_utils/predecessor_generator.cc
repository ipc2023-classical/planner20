...
void PredecessorGenerator::generate_applicable_ops(
    const PartialAssignment &assignment, vector<OperatorID> &applicable_ops, bool reject_unassigned) const {
    root->generate_applicable_ops(assignment.get_unpacked_values(), applicable_ops, reject_unassigned);
